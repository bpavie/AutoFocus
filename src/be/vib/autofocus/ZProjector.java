...
public class ZProjector {//implements PlugIn {
    ...
    private int method = (int)Prefs.get(METHOD_KEY, MAX_METHOD);
  ...
  public void setMethod(int projMethod){
    ...
    public void run(String arg, int[] channelArray) {
    imp = IJ.getImage();
    ...
    if (isHyperstack) {
      ...
      doHyperStackProjection(allTimeFrames, channelArray);
...
//    }
  ...
  public void doHyperStackProjection(boolean allTimeFrames, int[]channelArray) {
    ...
    for (int frame=firstFrame; frame<=lastFrame; frame++) {
      ...
      for (int channel=1; channel<=channels; channel++) {
        if(isPart(channelArray, channel))
        {
          //System.out.println("For channel :"+channel);
...
//      }
        ...
        projImage.setDimensions(channelArray.length, 1, frames);
        if (channels>1) {
  ...
  */
    ...
    private boolean isPart(int[] arr, int value) {
      for(int s: arr){
        if(s == value)
          return true;
      }
